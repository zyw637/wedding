...
(function () {
  ...
  const FRENCH_CHATEAU_BUNDLE = {
    ...
    hero: {
      ...
      subtitleEn: "A FRENCH CHATEAU WEDDING",
      names: "万卓洋 ＆ 张佳敏",
      namesEn: "ZHUOYANG WAN & JIAMIN ZHANG",
      poem: "晨光熹微，微风拂过庄园林荫。\n我们诚挚邀请您，共赴这一场关于爱与长情的盛宴。"
    },
    calendar: {
      year: "ANNO DOMINI 2026",
      days: "天",
      hours: "时",
      minutes: "分",
      seconds: "秒"
    ...
    gallery: {
      ...
      act1Title: "心动 · 晨曦油画",
      act1Quote: "初见心动，在油画般的光影里定格温柔。",
      act2Tag: "CHAPTER II · DEVOTION",
      act2Title: "相爱 · 绿茵光影",
      act2Quote: "我们在彼此的目光里，找到了同一个明朗远方。",
      act3Tag: "CHAPTER III · ETERNITY",
      act3Title: "余生 · 庭阁长情",
      act3Quote: "执子之手，从此朝暮并肩共赴余生。",
      swipeHint: "← 左右滑动 翻阅多图 →"
    },
    itinerary: {
      tag: "SCHEDULE",
  ...
  /* --- 2. ROMANTIC ROSE PETAL PHYSICS ENGINE --- */
  class PetalPhysicsEngine {
    ...
    initAmbientPetals() {
      for (let i = 0; i < 14; i++) {
        this.petals.push(this.createPetal(Math.random() * this.width, Math.random() * this.height, true));
      }
    }

    createPetal(x, y, ambient = false) {
      const colors = ['rgba(244, 219, 215, 0.75)', 'rgba(255, 235, 230, 0.8)', 'rgba(235, 200, 195, 0.7)', 'rgba(248, 238, 225, 0.8)'];
      return {
        x: x,
        y: y,
        vx: (Math.random() - 0.5) * 1.5,
        vy: ambient ? (Math.random() * 0.8 + 0.4) : (Math.random() * 2 + 1),
        ...
        alpha: ambient ? 0.75 : 1,
        decay: ambient ? 0 : 0.012
      };
    }

    spawnTouchPetals(x, y, count = 4) {
      for (let i = 0; i < count; i++) {
        const p = this.createPetal(x + (Math.random() - 0.5) * 20, y + (Math.random() - 0.5) * 20, false);
        this.petals.push(p);
      }
    }
    ...
    bindTouch() {
      ...
      window.addEventListener('click', (e) => {
        this.spawnTouchPetals(e.clientX, e.clientY, 5);
      });
    }
    ...
    loop() {
      ...
      for (let i = this.petals.length - 1; i >= 0; i--) {
        ...
        p.x += p.vx + Math.sin(p.oscillationStep) * 0.8;
        p.y += p.vy;
        ...
        this.ctx.beginPath();
        this.ctx.ellipse(0, 0, p.size * 0.55, p.size, Math.PI / 4, 0, Math.PI * 2);
        this.ctx.fill();
  ...
  class FrenchAudioPlayer {
    constructor() {
      this.audioUrl = 'https://assets.mixkit.co/music/preview/mixkit-romantic-moment-violin-piano-1011.mp3';
  ...
  class ShowcaseController {
        ...
        if (photos.length > 0) {
          ...
          if (frame) {
            ...
            frame.addEventListener('touchend', (e) => {
              const deltaX = this.showcases[scId].touchStartX - e.changedTouches[0].clientX;
              const deltaY = this.showcases[scId].touchStartY - e.changedTouches[0].clientY;
    ...
    prev(scId) {
      const sc = this.showcases[scId];
      if (!sc) return;
      const prevIdx = (sc.currentIndex - 1 + sc.photos.length) % sc.photos.length;
  ...
  class FrenchChateauApp {
    ...
    init() {
      ...
      this.countdown = new CalendarCountdown('2026-10-18T11:58:00');
